...
ADD COLUMN IF NOT EXISTS widget_width integer DEFAULT 380,
ADD COLUMN IF NOT EXISTS widget_height integer DEFAULT 420,
ADD COLUMN IF NOT EXISTS widget_open_animation text DEFAULT 'fade',
ADD COLUMN IF NOT EXISTS widget_close_animation text DEFAULT 'fade',
ADD COLUMN IF NOT EXISTS widget_show_once_session boolean DEFAULT false,
ADD COLUMN IF NOT EXISTS widget_device_target text DEFAULT 'all',
...
UPDATE public.projects SET 
  ...
  widget_width = COALESCE(widget_width, 380),
  widget_height = COALESCE(widget_height, 420),
  widget_open_animation = COALESCE(widget_open_animation, 'fade'),
  widget_close_animation = COALESCE(widget_close_animation, 'fade'),
  widget_show_once_session = COALESCE(widget_show_once_session, false),
  widget_device_target = COALESCE(widget_device_target, 'all'),
